...
public class UserController {
     ...
     */
    @PostMapping()
    ResponseEntity<Api> addRide(@RequestBody @Valid User p, Errors errors) {
        try {
            check(errors);
            return (userService.addUser(p)) ? ResponseEntity.status(HttpStatus.CREATED).body(new Api("Adding was successful!", HttpStatus.CREATED)) : ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new Api("Adding was NOT successful!!", HttpStatus.INTERNAL_SERVER_ERROR));
     ...
     */
    @PutMapping("{id}")
    public ResponseEntity<Api> putUser(@RequestBody @Valid User p, Errors errors, @PathVariable String id) {
        try {
            check(errors);
            User user = userService.getById(id);
            if (user != null) {
                userService.updateRide(user, p);
